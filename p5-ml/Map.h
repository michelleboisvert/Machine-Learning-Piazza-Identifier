...
private:
  ...
  class PairComp {
    private:
      Key_compare compareKeys;
    public:
      bool operator()(Pair_type pair1, Pair_type pair2) const{
        return compareKeys(pair1.first, pair2.first);
      }
  };
...
Map<K, V, C>::insert(const Pair_type &val) {
  std::pair<typename Map<K,V,C>::Iterator, bool> pair;
  //find bst val if end need to insert so true
