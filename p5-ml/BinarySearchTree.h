...
public:
  ...
  //testing function
  /*size_t tree_sway() const {
    return static_cast<size_t>(tree_sway_impl(root));
  }*/
...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  /*static int tree_sway_impl(const Node *node){
    if(node == nullptr)
      return 0;
    if(node->left != nullptr && node->right != nullptr)  
      return tree_sway_impl(node->left) + tree_sway_impl(node->right);
    if(node->left != nullptr)
      return -1 + tree_sway_impl(node->left);
    if(node->right != nullptr)
      return 1 + tree_sway_impl(node->right);
    return 0;
  }*/
